...
class tpch8{
    ...
    public static String cleanStatement(String sql) throws JSQLParserException {
        ...
        ExpressionDeParser expr = new tpch8.ReplaceLongValues();
    ...
    public static ArrayList<ArrayList<String>> getAllTuples(Statement stmt) throws Exception {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            attributeTypes.put(columnLabel, qStarPhiMD.getColumnTypeName(i));
        }
    ...
    public static ArrayList<String> getAllQueries(String query, Set<String> selectTables, HashMap<String, ArrayList> potentialJoins) {
        ...
        for(String selectTable: selectTables) {
            ...
            for(ArrayList<String> table: tables) {
                from = selectTable + ", " + table.get(0);
                where = selectTable + "." + table.get(1) + " = " + table.get(0) + "." + table.get(2);
                ArrayList<ArrayList<String>> joins = potentialJoins.get(table.get(0));
                if(joins != null) {
                    for (ArrayList<String> join : joins) {
                        from = from + ", " + join.get(0);
                        where = where + " AND " + table.get(0) + "." + table.get(2) + " = " + join.get(0) + "." + join.get(2);
                        newQuery = querySelect + " FROM " + from + " WHERE " + where;
                        queries.add(newQuery);
                    }
                }
    ...
    public static void main(String args[]) throws Exception {
        ...
        mutationAttrValues.add("part.p_retailprice -> 800 700 600 500; >");
        mutationAttrValues.add("partsupp.ps_supplycost -> 990 980 970 960; >");
        ...
        do {
            query = mutatedQueries.get(34); //rand.nextInt(mutatedQueries.size())
