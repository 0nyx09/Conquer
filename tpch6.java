...
* description: to implement the conquer algorithm for refining query 6 using the tpc-h dataset
* */
...
class tpch6{
    ...
    public static String cleanStatement(String sql) throws JSQLParserException {
        ...
        ExpressionDeParser expr = new tpch6.ReplaceLongValues();
    ...
    public static void main(String args[]) throws Exception {
        ...
        String originalQuery = "SELECT supplier.s_name FROM supplier, nation WHERE supplier.s_nationkey = nation.n_nationkey AND s_acctbal > 1000 AND n_regionkey < 5";
        System.out.println("Original Query: " + originalQuery);
        ArrayList<String> mutationAttrValues = new ArrayList<>();
        mutationAttrValues.add("supplier.s_acctbal -> 2000 3000 4000 5000; >");
        mutationAttrValues.add("nation.n_regionkey -> 1 2 3 4; <");
        int noOfMutations = mutationAttrValues.size();
        ...
        do {
            query = mutatedQueries.get(94); //rand.nextInt(mutatedQueries.size())
        ...
        double percentage = 1.0; //rand.nextFloat();
//        int size = s.size();
...
//        }
        ...
        for(int i = 0; i < s.size(); i++) {
            ArrayList<String> whyNot = s.get(i);
            if(!(whyNot.get(1).contains("1336") || whyNot.get(1).contains("9819"))) {
                s.remove(i);
                i--;
            }
        }
                ...
                protected void visitBinaryExpression(BinaryExpression expr) {
                    if (expr instanceof ComparisonOperator) {
                        if (!(expr.getRightExpression() instanceof Column)) {
                            selectionAttributes.add(expr.getLeftExpression().toString());
